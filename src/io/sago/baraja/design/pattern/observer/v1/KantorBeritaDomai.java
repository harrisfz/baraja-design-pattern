package io.sago.baraja.design.pattern.observer.v1;

public class KantorBeritaDomai implements Observer, BroadcastMedia {
    private SoekarnoHatta soekarnoHatta;
    private String statusKemerdekaan = "";

    public KantorBeritaDomai(SoekarnoHatta soekarnoHatta) {
        this.soekarnoHatta = soekarnoHatta;
        this.soekarnoHatta.addObserver(this);
    }

    @Override
    public void broadcast() {
        System.out.println("KantorBeritaDomai Menyiarkan melalui media kawat:" + statusKemerdekaan);
    }

    @Override
    public void update(String status) {
        this.statusKemerdekaan = status;
        broadcast();
    }
}
